#!/usr/bin/env python3
"""
PCA visualization of the learned latent space.
...
Usage:
  python pca_viz.py [--ckpt train_log/default/model/latest.pt] [--output pca_latent.png]
"""
...
def main():
    parser = argparse.ArgumentParser(description="PCA visualization of latent space")
    parser.add_argument("--ckpt", type=str, default="train_log/default/model/latest.pt")
    parser.add_argument("--output", type=str, default="pca_latent.png")
    ...
    print(f"Running PCA on {len(geom_all)} samples...")
    scaler = StandardScaler()
    geom_all_scaled = scaler.fit_transform(geom_all)
    pca = PCA(n_components=2, random_state=42)
    embedding = pca.fit_transform(geom_all_scaled)
    ...
    if len(paired_indices) > 0 and image_encoder is not None:
        ...
        combined_scaled = scaler_img.fit_transform(combined)
        combined_emb = pca_img.fit_transform(combined_scaled)
