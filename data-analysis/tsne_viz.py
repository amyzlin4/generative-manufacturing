...
Usage:
  python tsne_viz.py [--ckpt train_log/default/model/latest.pt] [--output tsne_latent.png]
"""
...
def main():
    parser = argparse.ArgumentParser(description="t-SNE visualization of latent space")
    parser.add_argument("--ckpt", type=str, default="train_log/default/model/latest.pt")
    parser.add_argument("--output", type=str, default="tsne_latent.png")
    ...
    print(f"Running t-SNE on {len(geom_all)} samples (perplexity={args.perplexity}, lr={args.lr}, n_iter={args.n_iter})...")
    tsne = TSNE(
        n_components=2,
    ...
    embedding = tsne.fit_transform(geom_all)
    ...
    if len(paired_indices) > 0 and image_encoder is not None:
        ...
        combined_emb = tsne_img.fit_transform(combined)
